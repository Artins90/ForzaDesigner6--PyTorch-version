...
Locator strategy:
  ...
  2. FALLBACK (legacy): dual-strategy u16-count scan. Used when the RTTI chain
     can't be resolved (class missing, etc.). This was FD6's only locator
     before the bvzrays-informed refactor, now upgraded to filter private heaps.

  In both paths, candidates are validated by the same strict 5/5 layer
  fingerprint + 85% full-table check before any write happens. RTTI just
...
def locate_livery_group(
    ...
    Dual-Strategy Search implementation:
      - Strategy Phase 1 (v1.3): Scans only committed, private regions <= 256 MB in 
        ascending virtual address order. This minimizes time spent searching 
        irrelevant, enormous heaps allocated by FH6.
      - Strategy Phase 2 (v1.4): Fallback scan for regions > 256 MB, sorted by size 
        descending to cover the remaining search space.
    """
    pattern = struct.pack('<H', layer_count)
    all_regions = []
    
    # 1. Enumerate and safely filter memory regions (MEM_COMMIT + MEM_PRIVATE, excluding PAGE_GUARD)
    for r in proc.enumerate_regions():
        if r.readable and r.writable and not r.is_image:
            # Safely check metadata parameters if exposed by the ProcessHandle representation
    ...
    # 2. Strategy Phase 1: Small/Medium regions (<= 256MB) in ascending virtual address order.
    small_regions = [r for r in all_regions if r.size <= 256 * 1024 * 1024]
    small_regions.sort(key=lambda r: r.base)  # address_asc order
    
    # 3. Strategy Phase 2: Giant regions fallback, sorted by size descending.
    large_regions = [r for r in all_regions if r.size > 256 * 1024 * 1024]
    large_regions.sort(key=lambda r: r.size, reverse=True)  # size_desc order
...
def _pack_color(shape_dict: dict, force_solid: bool = False) -> bytes:
    """Convert FD6 shape's color to RGBA 4 bytes, supporting dynamic opacity."""
    ...
    def inject(self, shapes: list, group: VinylGroupHandle, progress_cb=None,
        ...
        for i, sd in enumerate(shape_dicts):
            ...
            try:
                ...
                # Color: RGBA bytes with dynamic alpha/opacity supported
