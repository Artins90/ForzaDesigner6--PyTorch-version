...
class RightTriangle(Shape):
    ...
    def _get_vertices(self) -> list[tuple[float, float]]:
        ...
        # Right-angle aligned bottom-left relative to local shape scale
        v1 = rotate(-self.rx, -self.ry)  
    ...
    def mutate(self, rng: random.Random, w: int, h: int) -> "RightTriangle":
        ...
        if which == 0:
            ...
            new.angle = (new.angle + rng.gauss(0, 25)) % 360.0
        return new
    ...
    @classmethod
    def from_json(cls, data: dict) -> "RightTriangle":
        return cls(
    ...
    @classmethod
    def random(cls, rng: random.Random, w: int, h: int) -> "RightTriangle":
        return cls(
